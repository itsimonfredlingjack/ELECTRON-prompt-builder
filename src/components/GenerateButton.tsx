...
export function GenerateButton({ onClick, onStop, isStreaming, disabled }: GenerateButtonProps) {
  if (isStreaming) {
    return (
      <button
        onClick={onStop}
        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-medium text-ghost-bright rounded-lg surface transition-colors duration-200"
      >
